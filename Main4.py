    ...
    def mouseReleaseEvent(self, event):
        if event.button() == Qt.MouseButton.MiddleButton:
            ...
            if self.autofill_direction == 'vertical':
                ...
                for i, r in enumerate(fill_range):
                    for c in range(sel_cols):
                        # Reverse-start index if dragging up
                        value_index = i % sel_rows if step == 1 else (-1 - i) % sel_rows
                        value = selected_values[value_index][c]
                        model.setData(model.index(r, sel_left + c), value)
                ...
                for i, c in enumerate(fill_range):
                    for r in range(sel_rows):
                        # Reverse-start index if dragging left
                        value_index = i % sel_cols if step == 1 else (-1 - i) % sel_cols
                        value = selected_values[r][value_index]
                        model.setData(model.index(sel_top + r, c), value)

            # Reset autofill state
            # After applying autofill
            # Determine the area to keep visual
